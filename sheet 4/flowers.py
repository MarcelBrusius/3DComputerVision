...
def ncc(patch_1, patch_2):
	sol = np.ndarray(3)	
	for i in range(3):
		vec1 = np.reshape(patch_1[:,:,i],(len(patch_1[:,:,i].flat),1))
		vec2 = np.reshape(patch_2[:,:,i],(len(patch_2[:,:,i].flat),1))

		norm1 = np.linalg.norm(vec1,2)
		norm2 = np.linalg.norm(vec1,2)
		...
		if (norm1 == 0 and norm2 == 0):
			...
			sol[i] = np.divide(dot,norm1*norm2)
	return sol.mean()
...
def main():
	...
	#L = l_im[H_BOUND[0]:H_BOUND[1],W_BOUND[0]:W_BOUND[1]]
